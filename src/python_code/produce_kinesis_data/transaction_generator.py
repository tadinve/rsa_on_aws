...
def genrateKinesisSaleRecord(products_info, invoiceNo, stockCode, invoiceDate, fake):
...
def generateTransactions(startDate=datetime(2018,1,1), endDate=datetime(2020,12,31), n=10, OutFile=None):
...
def generateKinesisTransactions(startDate=datetime(2018,1,1), endDate=datetime(2020,12,31), n=10, OutFile=None):
    ...
    for i in range(n):
        stockCode = prods[i]
        products_info = df.loc[stockCode]
        kRec = genrateKinesisSaleRecord(products_info, i, stockCode, invoiceDate, fake)
        kinesisRecords.append(kRec)
        invoiceDate += delta
